...
##################################
"This enables a FastAPI server for the house-insurance-service microservice."
...
KAFKA_TOPIC = "user.house.insurance.purchased"
...
async def lifespan(app: FastAPI):
    ...
    """
    logger.info("=" * 60)
    logger.info("House Insurance Service starting up...")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info("✅ House Insurance Service ready to accept requests")
    logger.info("=" * 60)
    
    yield
    
    logger.info("=" * 60)
    logger.info("House Insurance Service shutting down...")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info("✅ House Insurance Service shutdown complete")
    logger.info("=" * 60)

app = FastAPI(lifespan=lifespan, title="House Insurance Service", version="1.0.0")
...
def read_root():
    ...
    return {"message": "House Insurance Service is running"}
...
async def post_house_insurance_contract(contract_data: ContractRequest, db: Session = Depends(get_db)):
    ...
    logger.info(f'API call: /widget/house-insurance/contract/{user_id}/{widget_id} starts')
...
@app.get("/widget/house-insurance/contract/{user_id}")
def get_house_insurance_contracts(user_id: int, db: Session = Depends(get_db)):
    '''
    ...
    :type db: Session
    '''
    logger.info(f'API call: /widget/house-insurance/contract/{user_id} starts')
...
@app.delete("/widget/house-insurance/contract/{user_id}/{widget_id}")
async def delete_house_insurance_contract(user_id: int, widget_id: str, db: Session = Depends(get_db)):
    """
    ...
    """
    logger.info(f'API call: DELETE /widget/house-insurance/contract/{user_id}/{widget_id} starts')
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Could not delete contract in house insurance service database (DB Connection Error)."
        )

if __name__ == "__main__":
    import uvicorn
    logger.info("Starting House Insurance Service with Uvicorn...")
